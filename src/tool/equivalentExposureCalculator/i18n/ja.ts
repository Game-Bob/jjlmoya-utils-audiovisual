...
const seo: ToolLocaleContent<EquivalentExposureUI>['seo'] = [
  ...
  {
    type: 'paragraph',
    html: '写真における露出は絞り(f値)、シャッタースピード、ISO感度の3つの要素の相互作用によって決定されます。どれか1つの要素を変更した場合、同じ明るさ(EV値)を維持するためには他の要素を相回転させて正確に補正する必要があります。',
  },
  {
    type: 'stats',
    items: [
  ...
  {
    type: 'paragraph',
    html: '同等の露出値(EV)に設定してセンサーへの総光量を同じに保った場合でも、写真の視覚的な表現や被写体のボケ味・動きの描写は大きく変化します：',
  },
  {
    type: 'list',
    items: [
